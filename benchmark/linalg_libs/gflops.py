...
if __name__ == "__main__":
    ...
    pt_sec = timeit(lambda: pt.matmul(pt_A, pt_B) + pt_A *
                    pt_B * pt.matmul(pt_A, pt_B), number=3)/3
    ...
    tf_sec = timeit(lambda: tf.matmul(tf_A, tf_B) + tf_A *
                    tf_B * tf.matmul(tf_A, tf_B), number=3)/3
